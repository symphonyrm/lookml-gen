...
from lookmlgen import base_generator, field, view
...
def test_pdt_view():
    ...
    v = view.View(testname)
    v.derived_table = pdt
    v.add_field(field.Dimension("id", type="number", primary_key=True))
...
def test_dimension_group_no_timeframes():
    ...
    v.add_field(field.DimensionGroup("dimension1", sql="${TABLE}.dim1"))
    f = six.StringIO()
    fo_omit_timeframes = base_generator.GeneratorFormatOptions(
...
def test_newlines():
    ...
    for l in ["a", "b", "c", "d"]:
        ...
        v.add_field(
            field.Measure("sum_" + l, type="sum", sql="${{{0}}}".format(l))
        )
